> # 2025/08/01
> 1. 保留高分辨率掩码，并对SegmentResult类进行了调整；
> 2. 重构了视觉检测相关代码，形成了基于类的ComputePose.py文件；
> 3. 针对实际调试进行了相关优化，将重要参数集成到Const类中，在检测过程中将相关检测结果绘制在show_img上；
> 4. 相关代码进行了初步检测测试，但仍需要全面测试。
